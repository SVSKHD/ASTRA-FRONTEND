...
import { X, Calendar, Flag, AlignLeft, Trash2, Clock } from "lucide-react";
...
export const TaskDialog = ({ isOpen, onClose, onSave, onDelete, initialTask }: TaskDialogProps) => {
    ...
    useEffect(() => {
        if (isOpen) {
            if (initialTask) {
                ...
                // Assuming deadline is a Firestore timestamp or null
                if (initialTask.deadline) {
                    // Check if it's a Firestore timestamp (has toDate) or string/date
                    const d = initialTask.deadline.toDate ? initialTask.deadline.toDate() : new Date(initialTask.deadline);
                ...
                // Create Mode (not used yet given current flow, but good to have)
                setContent("");
                setDescription("");
                setPriority("Medium");
                setDeadline("");
            ...
            >
                ...
                <div className="flex items-center justify-between p-6 border-b border-white/5 bg-white/5">
                    <div className="flex items-center gap-2 text-white/50 text-sm font-medium uppercase tracking-wider">
                        <span className={`w-2 h-2 rounded-full ${priority === 'High' ? 'bg-red-400' : priority === 'Medium' ? 'bg-yellow-400' : 'bg-blue-400'
                            }`} />
                        {priority} Priority
                    </div>
