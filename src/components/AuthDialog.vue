...
const { s } = useStyles()
const { authOpen } = storeToRefs(auth)
</script>
...
<template>
  <template v-if="authOpen">
    ...
    <div :style="s.authCard">
      ...
      <span :style="s.finMeta">Your universe, tracked. Sign in to sync across devices.</span>
      <div :style="s.authRow">
        <button :style="s.authBtn" v-hover-style="s.addBtnHover" aria-label="Continue with Google" @click="auth.loginGoogle()">
        ...
        <button :style="s.authBtn" v-hover-style="s.addBtnHover" aria-label="Continue with GitHub" @click="auth.loginGithub()">
      ...
      <button :style="s.authGuest" @click="auth.continueGuest()">Continue as guest</button>
    </div>
  </template>
</template>
